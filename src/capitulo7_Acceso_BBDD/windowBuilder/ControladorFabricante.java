...
public class ControladorFabricante {
	...
	public ControladorFabricante() {
		...
		try {
			...
			conn = (Connection) DriverManager
					.getConnection("jdbc:mysql://127.0.0.1/tutorialjavacoches?serverTimezone=UTC", "java", "123a");
		} catch (Exception e) {
	...
	public Fabricante findSiguiente(int idActual) {
		...
		try {
			...
			ResultSet rs = s.executeQuery(
					"select * from tutorialjavacoches.fabricante where id > " + idActual + " order by id limit 1");
			if (rs.next()) {
				f = new Fabricante();
				f.setId(rs.getInt("id"));
				f.setCif(rs.getString("cif"));
	...
	public Fabricante findAnterior(int idActual) {
		...
		try {
			Statement s = this.conn.createStatement();
			ResultSet rs = s.executeQuery(
					"select * from tutorialjavacoches.fabricante where id < " + idActual + " order by id desc limit 1");
			if (rs.next()) {
				f = new Fabricante();
				f.setId(rs.getInt("id"));
				f.setCif(rs.getString("cif"));
	...
	public int modificar(Fabricante f) {
		...
		try {
			...
			registrosAfectados = s.executeUpdate("update fabricante set cif='" + f.getCif() + "', " + " nombre='"
